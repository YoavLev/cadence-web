...
describe('getLastProgressFromHistory', () => {
  it('decodes the last heartbeat from the newest activity timeout event', () => {
    expect(
      getLastProgressFromHistory(
        mockBatcherActivityTimedOutHistory.history?.events
      )
    ).toEqual({
      totalEstimate: MOCK_BATCH_PROGRESS.TotalEstimate,
      successCount: MOCK_BATCH_PROGRESS.SuccessCount,
  ...
  it('does not use MOCK_LAST_BATCH_PROGRESS (the earlier timeout) when a newer one exists', () => {
    expect(
      getLastProgressFromHistory(
        mockBatcherActivityTimedOutHistory.history?.events
      )?.successCount
    ).not.toBe(MOCK_LAST_BATCH_PROGRESS.SuccessCount);
  });

  it('returns undefined for a workflow-level timeout with no activity timeout event', () => {
    expect(
      getLastProgressFromHistory(
        mockBatcherWorkflowTimedOutHistory.history?.events
      )
    ).toBeUndefined();
  });

  it('returns undefined when events is undefined', () => {
    expect(getLastProgressFromHistory(undefined)).toBeUndefined();
  });
});
